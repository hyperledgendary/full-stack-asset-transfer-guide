...
// Path to crypto materials. Convenience for when run using fabric samples test-network using credentials generated by cryptogen
const cryptoPath = path.resolve(process.env.CRYPTO_PATH ?? path.resolve(__dirname, '..', '..', '..', '..', 'fabric-samples', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com'));

// Path to user private key directory.
const privateKeyPath = path.resolve(process.env.PRIVATE_KEY_PATH ?? locatePrivateKeyFile());
...
function locatePrivateKeyFile(): string {
